...
def _fixed_iniciar_cmd() -> Path:
    local = os.environ.get("LOCALAPPDATA", "")
    return Path(local) / "cursor-usage-tray" / "app" / "Iniciar.cmd"


def app_executable_path() -> str:
    fixed = _fixed_iniciar_cmd()
    if fixed.exists():
        return str(fixed.resolve())
    if getattr(sys, "frozen", False):
        tray_dir = Path(sys.executable).resolve().parent
        iniciar = tray_dir / "Iniciar.cmd"
        if iniciar.exists():
            return str(iniciar.resolve())
        return str(Path(sys.executable).resolve())
    return str((Path(__file__).resolve().parent.parent / "run-tray.ps1").resolve())
