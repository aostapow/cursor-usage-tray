...
class TaskbarLabel:
    ...
    def _persist_position(self) -> None:
        ...
        if self._saved_x == x and self._saved_y == y:
            return
        self._saved_x = x
        self._saved_y = y
    ...
    def _schedule_persist_position(self) -> None:
        if self._closed:
            return
        self._cancel_persist_job()

        def _save() -> None:
            self._persist_job = None
            self._persist_position()

        self._persist_job = self.root.after(POSITION_SAVE_DEBOUNCE_MS, _save)

    def _flush_persist_position(self) -> None:
        self._cancel_persist_job()
        self._persist_position()
